...
param(
    ...
    [int]$Number = 0,
    [switch]$Help,
    [string]$DescriptionFilePath, # New parameter for file path
...
if ($Help) {
    Write-Host "Usage: ./create-new-feature.ps1 [-Json] [-ShortName <name>] [-Number N] [-DescriptionFilePath <path>] <feature description>"
    Write-Host ""
    Write-Host "Options:"
    Write-Host "  -Json               Output in JSON format"
    Write-Host "  -ShortName <name>   Provide a custom short name (2-4 words) for the branch"
    Write-Host "  -Number N           Specify branch number manually (overrides auto-detection)"
    Write-Host "  -DescriptionFilePath <path>  Path to a file containing the feature description"
    Write-Host "  -Help               Show this help message"
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  ./create-new-feature.ps1 'Add user authentication system' -ShortName 'user-auth'"
    Write-Host "  ./create-new-feature.ps1 -DescriptionFilePath './feature.txt' -ShortName 'user-auth'"
    exit 0
}
...
if ($DescriptionFilePath) {
    if (Test-Path $DescriptionFilePath) {
        ...
        Write-Error "Error: Description file not found at '$DescriptionFilePath'"
        exit 1
    }
} elseif (-not $FeatureDescription -or $FeatureDescription.Count -eq 0) {
    Write-Error "Usage: ./create-new-feature.ps1 [-Json] [-ShortName <name>] [-DescriptionFilePath <path>] <feature description>"
    exit 1
} else {
